...
class SemiSolidPreSolver implements PreSolver {
	...
	public boolean preSolve(Object otherUserData) {
		...
		if(myAgent != null && otherAgent != null && myAgent.getUserData() instanceof Role &&
				otherAgent.getUserData() instanceof Role) {
			// If both Agents have bounds, and Agent's top bound is below other Agent's bottom bound,
			// then remove semi-solid bit.
			Rectangle myAgentBounds = RP_Tool.getBounds((Role) myAgent.getUserData());
			Rectangle otherAgentBounds = RP_Tool.getBounds((Role) otherAgent.getUserData());
