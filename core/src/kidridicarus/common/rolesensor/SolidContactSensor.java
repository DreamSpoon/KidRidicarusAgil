...
public class SolidContactSensor extends AgentContactSensor {
	...
	private LinkedList<Role> roleContacts;

	public SolidContactSensor(Role parentRole) {
		super(parentRole == null ? null : parentRole.getAgent());
		lineSegContacts = new LinkedList<SolidLineSeg>();
		roleContacts = new LinkedList<Role>();
	}
	...
	public void onBeginSense(AgentBodyFilter abf) {
		...
		if(agent != null && agent.getUserData() instanceof SolidRole && !roleContacts.contains(agent.getUserData()))
			roleContacts.add((Role) agent.getUserData());
	}
	...
	public void onEndSense(AgentBodyFilter abf) {
		...
		Agent agent = AgentBodyFilter.getAgentFromFilter(abf);
		if(agent != null && agent.getUserData() instanceof SolidRole && roleContacts.contains(agent.getUserData()))
			roleContacts.remove((Role) agent.getUserData());
	}
	...
	public boolean isContacting() {
		return !roleContacts.isEmpty() || !lineSegContacts.isEmpty();
	}

	public boolean isDirSolid(Direction4 dir, Rectangle testBounds) {
		return isDirBlockedByLineSeg(dir, testBounds) || isDirBlockedByRole(dir, testBounds);
	}
	...
	private boolean isDirBlockedByRole(Direction4 dir, Rectangle testBounds) {
		if(dir == null || dir == Direction4.NONE)
			throw new IllegalArgumentException("dir must be non null and not NONE, but dir="+dir);
		for(Role role : roleContacts) {
			// Get other Role's bounds, skipping other Role if bounds are null. If other is too far above or
			// below testBounds then skip Role because contact is impossible.
			Rectangle otherBounds = RP_Tool.getBounds(role);
